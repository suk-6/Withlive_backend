...
async def serverHandler(websocket, path):
    ...
    while True:
        ...
        for bbox in zip(results.xyxy[0]):
            ...
            if yPositions[0] <= yCenter < yPositions[1]:  # 중단
                if xCenter < imageWidth / 2:  # 왼쪽 중단
                    bboxCoords["position"] = 2
                elif xCenter > imageWidth / 2:  # 오른쪽 중단
                    bboxCoords["position"] = 3
            elif yPositions[1] <= yCenter < yPositions[2]:  # 하단
                if xCenter < imageWidth / 2:  # 왼쪽 하단
                    bboxCoords["position"] = 4
                elif xCenter > imageWidth / 2:  # 오른쪽 하단
                    bboxCoords["position"] = 5
            elif yCenter < yPositions[0]:  # 상단
                if xCenter < imageWidth / 2:  # 왼쪽 상단
                    bboxCoords["position"] = 0
                elif xCenter > imageWidth / 2:  # 오른쪽 상단
